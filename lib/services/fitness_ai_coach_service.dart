...
// Calls GeminiService through the Cloudflare Worker and stores feedback on the
// activity doc.
...
class FitnessAICoachService {
  ...
  Future<String> generateAndSaveFeedback(FitnessActivityModel activity) async {
    ...
    if (!(_featureFlags?.fitnessAiFeedbackReady ?? false)) {
      return '';
    }
  ...
  static const _kSystemPrompt =
      'You are the user\'s personal fitness coach inside the Optivus app. '
  ...
  Future<String> _buildPrompt(FitnessActivityModel activity) async {
    ...
    if (activity.averagePaceSecondsPerKm != null &&
      ...
      buf.writeln(
          "Avg Pace: ${p ~/ 60}'${(p % 60).round().toString().padLeft(2, '0')}\" /km");
    }
    if (activity.calories != null) {
      buf.writeln('Calories: ${activity.calories}');
    }
    if (activity.averageHeartRate != null) {
      buf.writeln('Avg HR: ${activity.averageHeartRate} bpm');
    }
    ...
    if (goals.isNotEmpty) {
      ...
      for (final g in goals) {
        buf.writeln(
            '- ${g.goalType}: ${g.currentValue}/${g.targetValue} ${g.unit}');
      }
    }
    return buf.toString();
  }
